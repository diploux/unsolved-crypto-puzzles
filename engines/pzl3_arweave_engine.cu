 ...
 */
/*
 * pzl3_arweave_engine.cu - CUDA engine for Arweave Puzzle #3 (~1000 AR).
 *
 * Per-attempt pipeline (validated byte-for-byte against the live puzzle
 * page's own client-side decoder and its test oracle):
 *
 *   passphrase P  = concat of 8 lowercase 4-char tokens (32 ASCII bytes)
 ...
 *   - pzl3_selftest()         : on-device validation of 1 known vector (host-fed)
 *   - pzl3_init(salt,c0)      : load 8-byte salt + 16-byte cipher block 0
 *   - pzl3_run_index_range()  : evaluate a contiguous mixed-radix index range
 ...
 */
__device__ void pzl3_eval(const uint8_t P[32], const uint8_t *salt, const uint8_t *c0, uint8_t p0[16]) {
...
#define MAX_TOK 256
__constant__ char   c_words[8][MAX_TOK][4];
__constant__ int    c_wlen[8];          /* count per slot */

/* "Free slot" support: one slot (0..7) may be GENERATED from a charset instead
 * of read from a stored wordlist. The 4-char token of the free slot is the
 * base-c_cslen encoding of `sel` over the charset c_cs[]. This lets a slot span
 * the full [a-z0-9]^4 = 1,679,616 space without storing 1.6M tokens in constant
 * memory. c_free_k == -1 means "no free slot" (pure-wordlist mode, unchanged). */
__constant__ int    c_free_k;           /* free slot index 0..7, or -1 if none */
__constant__ char   c_cs[64];           /* charset for the free slot */
__constant__ int    c_cslen;            /* charset length (radix of the free slot) */

/* Build the 32-byte passphrase for a given mixed-radix global index. */
__device__ __forceinline__ void index_to_pass(uint64_t idx, uint8_t P[32]) {
    int free_k = c_free_k;
    int cslen  = c_cslen;
    #pragma unroll
    for (int k = 0; k < 8; k++) {
        if (k == free_k) {
            /* Generated slot: n = cslen^4; decode the 4 chars of sel in base cslen.
             * P[k*4+j] = c_cs[(sel / cslen^j) % cslen] (j=0 is the least-significant
             * digit -> first char). Successive division peels one base-cslen digit. */
            int n = cslen * cslen * cslen * cslen;
            int sel = (int)(idx % (uint64_t)n);
            idx /= (uint64_t)n;
            #pragma unroll
            for (int j = 0; j < 4; j++) {
                P[k*4+j] = (uint8_t)c_cs[sel % cslen];
                sel /= cslen;
            }
        } else {
            int n = c_wlen[k];
            int sel = (int)(idx % (uint64_t)n);
            idx /= (uint64_t)n;
            P[k*4+0] = (uint8_t)c_words[k][sel][0];
            P[k*4+1] = (uint8_t)c_words[k][sel][1];
            P[k*4+2] = (uint8_t)c_words[k][sel][2];
            P[k*4+3] = (uint8_t)c_words[k][sel][3];
        }
    }
}
...
__global__ void pzl3_range_kernel(uint64_t start, uint64_t count) {
    ...
    for (uint64_t t = blockIdx.x*blockDim.x + threadIdx.x; t < count; t += stride) {
        uint8_t P[32];
        index_to_pass(start + t, P);
        uint8_t p0[16];
        pzl3_eval(P, c_salt, c_c0, p0);
...
pzl3_bench_kernel(uint64_t base, uint64_t count,
 ...
 */
__global__ void pzl3_selftest_kernel(const uint8_t *P, const uint8_t *salt,
                                     const uint8_t *c0, uint8_t *out_p0) {
    if (blockIdx.x==0 && threadIdx.x==0) {
        uint8_t Pl[32], sl[8], cl[16], p0[16];
        #pragma unroll
        for (int i=0;i<32;i++) Pl[i]=P[i];
        #pragma unroll
        for (int i=0;i<8;i++) sl[i]=salt[i];
        #pragma unroll
        for (int i=0;i<16;i++) cl[i]=c0[i];
        pzl3_eval(Pl, sl, cl, p0);
        #pragma unroll
...
int pzl3_init(const uint8_t *salt8, const uint8_t *c0_16) {
    cudaError_t e;
    ...
    int free_none = -1;
    e = cudaMemcpyToSymbol(c_free_k, &free_none, sizeof(int)); if (e) return -4;
    return 0;
}

/*
 * Configure the "free slot": slot `free_k` (0..7) is GENERATED from charset
 * `cs` (cslen chars, <=64) as [cs]^4, rather than read from its wordlist. The
 * other 7 slots keep their wordlists. Pass free_k = -1 to disable (back to pure
 * wordlist mode). Call AFTER pzl3_set_wordlists (it does not touch c_words).
 */
int pzl3_set_free(int free_k, const char *cs, int cslen) {
    cudaError_t e;
    if (cslen < 0)  cslen = 0;
    if (cslen > 64) cslen = 64;
    e = cudaMemcpyToSymbol(c_free_k, &free_k, sizeof(int));  if (e) return -1;
    e = cudaMemcpyToSymbol(c_cslen,  &cslen,  sizeof(int));  if (e) return -2;
    char buf[64];
    for (int i = 0; i < 64; i++) buf[i] = (i < cslen) ? cs[i] : 0;
    e = cudaMemcpyToSymbol(c_cs, buf, 64);                   if (e) return -3;
    return 0;
}

/* Load 8 wordlists; words is 8*MAX_TOK*4 bytes, counts is int[8]. */
int pzl3_set_wordlists(const char *words, const int *counts) {
    cudaError_t e;
    e = cudaMemcpyToSymbol(c_words, words, sizeof(char)*8*MAX_TOK*4); if (e) return -1;
    e = cudaMemcpyToSymbol(c_wlen, counts, sizeof(int)*8);           if (e) return -2;
    return 0;
}
 ...
 */
long long pzl3_run_index_range(uint64_t start, uint64_t count, int threads, int blocks) {
    long long none = -1;
...
/*
 * pzl3_bench - RAW SPEED benchmark on synthetic inputs. Launches
 * pzl3_bench_kernel with `blocks`x`threads`, evaluating `count` synthetic
 ...
 */
int pzl3_bench(uint64_t base, uint64_t count, int threads, int blocks,
    ...
    cudaEventRecord(e0);
    pzl3_bench_kernel<<<blocks, threads>>>(base, count, dS, dC);
 ...
 */
int pzl3_selftest(const uint8_t *P32, const uint8_t *salt8, const uint8_t *c0_16, uint8_t *out_p0) {
    uint8_t *dP,*dS,*dC,*dO;
    if (cudaMalloc(&dP,32)) return -1;
    if (cudaMalloc(&dS,8))  return -1;
    if (cudaMalloc(&dC,16)) return -1;
    if (cudaMalloc(&dO,16)) return -1;
    cudaMemcpy(dP, P32,   32, cudaMemcpyHostToDevice);
    cudaMemcpy(dS, salt8, 8,  cudaMemcpyHostToDevice);
    cudaMemcpy(dC, c0_16, 16, cudaMemcpyHostToDevice);
    pzl3_selftest_kernel<<<1,1>>>(dP, dS, dC, dO);
    cudaError_t e = cudaDeviceSynchronize();
